...
//    var requestLaodingCalss: AnyObject.Type? = nil
    ...
    ///取请求缓存时用到的Chache对象
    let networkDiskCache: Storage = {
        return try! Storage(
            diskConfig: DiskConfig(name: kWXNetworkResponseCacheKey),
            memoryConfig: MemoryConfig(),
            transformer: TransformerFactory.forData()
        ).transformCodable(ofType: String.self)
    }()
    ...
    ///清除所有缓存
    func clearWXNetworkAllRequestCache(completion: @escaping (Bool) -> ()) {
        networkDiskCache.async.removeAll(completion: { result in
            switch result {
            case .value(_):
                completion(true)
            case .error(_):
    ...
    ///清除指定缓存
    func clearWXNetworkCacheOfRequest(serverApi: WXRequestApi, completion: @escaping (Bool)->()) {
        networkDiskCache.async.removeObject(forKey: serverApi.cacheKey) { result in
            switch result {
            case .value(_):
                completion(true)
            case .error(_):
